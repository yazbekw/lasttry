...
Version 7.0.0 - 10 Indicators (4 base + 6 relative) + Multi-Exchange
...
# Maximum theoretical score (must match the sum of individual maxima)
# trend 1.5 + momentum 1.0 + volume 1.0 + structure 1.0
# rel_rsi 0.5 + rel_strength 0.75 + vol_momentum 0.5
# atr_volatility 0.3 + bb_position 0.75 + macd 0.5
MAX_TOTAL_SCORE = 7.8
...
class IndicatorCalculator:
    ...
    def atr(highs: List[float], lows: List[float], closes: List[float],
        ...
        for i in range(1, len(closes)):
            ...
            trs.append(tr)
        # Simple moving average of TR for simplicity
        atr_values = []
    ...
    def relative_rsi_score(coin_rsi: Optional[float], market_avg_rsi: float) -> float:
        """Range: -0.5 .. +0.5
        Compares coin's RSI to market average.
        If coin RSI < market avg → more room to rise → positive.
        """
        if coin_rsi is None or market_avg_rsi <= 0:
            return 0.0
        try:
            diff = market_avg_rsi - coin_rsi  # positive if coin is "behind"
            if diff > 10:
                return 0.5
            if diff > 5:
                return 0.25
    ...
    def relative_strength_score(coin_change_24h: float, btc_change_24h: float) -> float:
        """Range: -0.75 .. +0.75
        Compares coin's 24h change to BTC's 24h change.
        """
    ...
    def volume_momentum_score(volumes: List[float]) -> float:
        """Range: -0.5 .. +0.5
        Compares current volume to volume 5 candles ago.
        """
        if len(volumes) < 6:
            return 0.0
        try:
    ...
    def atr_volatility_score(highs: List[float], lows: List[float],
        ...
        """Range: -0.3 .. +0.3
        Compares current ATR to ATR 20 candles ago.
        """
        try:
    ...
    def bollinger_position_score(close_prices: List[float],
        ...
        Low position (near lower band) → +0.75 (buy opportunity).
        High position (near upper band) → -0.5 (overbought).
        """
        ...
        try:
            ...
            # Inverted: low position → positive score (buy zone)
            if position < 0.1:
                return 0.75
            if position < 0.25:
                return 0.5
    ...
    def macd_histogram_score(close_prices: List[float]) -> float:
        """Range: -0.5 .. +0.5
        MACD histogram sign and crossover detection.
        """
        try:
            macd_line, signal_line, hist = IndicatorCalculator.macd(close_prices)
            ...
            # Crossed from negative to positive recently (bullish)
            ...
            if prev >= 0 and current < 0:
                return -0.5
            # Already negative and decreasing
            if current < 0 and current < prev:
                return -0.25
            return 0.0
        except Exception:
...
class SignalProcessor:
    ...
    def _build_result(total: float, raw_scores: Dict[str, float],
        ...
        # Per-indicator normalization factors (max theoretical per indicator)
        max_per_indicator = {
        ...
        for name, raw in raw_scores.items():
            max_val = max_per_indicator.get(name, 1.0)
            norm_pct = (raw / max_val) * 100.0 if max_val else 0.0
...
# ======================================================================
# Signal Manager (two-pass with relative indicators)
# ======================================================================
class SignalManager:
    def __init__(self):
    ...
    # ------------------------------------------------------------------
    # PASS 1: fetch data and compute base indicators + raw metrics
    # ------------------------------------------------------------------
    def _fetch_base(self, coin: CoinConfig) -> Optional[Dict]:
        ...
        # RSI value (for relative calculation later)
    ...
    # ------------------------------------------------------------------
    # PASS 2: compute relative indicators and finalize signals
    # ------------------------------------------------------------------
    ...
    def update_all(self) -> bool:
        with self.lock:
            coins = get_coins()
            logger.info(f"Updating {len(coins)} coins (discriminating mode)...")
            ...
            rsi_values = [d['rsi_value'] for d in intermediate
                          if d['rsi_value'] is not None]
...
def api_config_update():
    try:
        ...
        # Clear old notifications when settings change
        signal_manager.notification_manager.clear_history()
...
def send_startup_notification():
    try:
        ...
        keys = get_exchange_keys()
        msg = (
            f"Crypto Discriminating Analyzer Started (v7.0)\n"
...
if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("Crypto Discriminating Analyzer v7.0.0")
